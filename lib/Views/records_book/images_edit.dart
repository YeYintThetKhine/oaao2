...
class ImagesEdit extends StatefulWidget {
  final imglist;
  final record;
  ImagesEdit({@required this.imglist, @required this.record});

  _ImagesEditState createState() =>
      _ImagesEditState(imglist: imglist, record: record);
}
...
class _ImagesEditState extends State<ImagesEdit> {
  ...
  _ImagesEditState({@required this.imglist, @required this.record});

  var txtStyle = TextStyle(fontSize: 16.0, fontWeight: FontWeight.bold);
  ...
  bool _submit() {
    for (ImageData imgData in imglist) {
      db.updateImages(imgData: imgData, recid: record.recid);
    }
    return true;
  }
