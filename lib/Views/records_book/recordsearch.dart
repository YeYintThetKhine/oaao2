  ...
  Widget _buildList(){
    if(!(_searchText.isEmpty)){
      ...
      for(int i = 0;i<filteredProblems.length;i++){
        if(filteredProblems[i].problem.toLowerCase().contains(_searchText.toLowerCase())){
    ...
    return ListView.builder(
      ...
      itemBuilder:(context,i){
        return Card(
          ...
          elevation: 2.0,
          margin: EdgeInsets.only(left:12.0,right:12.0,top:6.0,bottom:6.0),
          child: InkWell(
            child:Container(
    ...
    return Scaffold(
      ...
      body:Container(
        child:Column(
          children: <Widget>[
            Container(
              padding: EdgeInsets.only(left:12.0,right:12.0,top:10.0,bottom:8.0),
              child:TextField( 
              controller: keyword,
              decoration: InputDecoration(              
                ...
                labelText: lan=='en'?'Search By Problem':'မှတ်တမ်းဖြင့်ရှာဖွေပါ'  
              ),
            ),
            ),
            Expanded(
